...
namespace Dex1WayConverted
{
    public class Contract1 : SmartContract
    {
        ...
        [Appcall("a749d459fd5cb08427ab37f99101ce92140552ee")] // ScriptHash
        public static extern int sellerToken1(string arg, object[] args);

        [Appcall("ad0363ca52951205a2711681b7ebeac8da6e117e")] // ScriptHash
        public static extern int sellerToken2(string arg, object[] args);

        [Appcall("c41e270805c2b82b35784a8bcfab9270beae3036")] // ScriptHash
        public static extern int buyerToken1(string arg, object[] args);

        [Appcall("91986337cfbb6f4ed4eeca89b2221c21386fc8ce")] // ScriptHash
        public static extern int buyerToken2(string arg, object[] args);
        ...
        public static Object Main(string oper, params Object[] args)
        {
            ...
            */
            if (oper == "oneWayFulfillPO")
            {
                ...
                return oneWayFulfillPO(_sellerTokens, _buyerTokens, _sellerValues, _buyerValues, _orderAddresses, _orderValues,_orderID);
            }
        ...
        {
            Runtime.Notify("in transfer tokens");
            var arg = new Object[] { };

            arg = new Object[] { _orderAddresses[1], _orderAddresses[2], _sellerValues[0] };
            sellerToken1("transfer", arg);

            arg = new Object[] { _orderAddresses[1], _orderAddresses[2], _sellerValues[1] };
            sellerToken2("transfer", arg);

            arg = new Object[] { _orderAddresses[2], _orderAddresses[1], _buyerValues[0] };
            buyerToken1("transfer", arg);

            arg = new Object[] { _orderAddresses[2], _orderAddresses[1], _buyerValues[1] };
            buyerToken2("transfer", arg);
            Runtime.Notify("tokens transfered");
        ...
        public static BigInteger calcTradeFee(BigInteger values, uint feeIndex)
        {
            ...
            if (feeIndex >= 0 && feeIndex <= 2 && values > 0)
            {
                ...
                Runtime.Notify("CALC TRADE FEE");
                BigInteger I = new BigInteger(check);
                totalFees = (I * values);///(1 NEO);
        ...
        public static bool oneWayFulfillPO(byte[][] _sellerTokens, byte[][] _buyerTokens, BigInteger[] _sellerValues, BigInteger[] _buyerValues, byte[][] _orderAddresses,
                BigInteger[] _orderValues, byte[] _orderID)// uint[] _v, byte[] _br, byte[] _bs, byte[] _sr, byte[] _ss, byte[] _orderID)
        {
            Runtime.Notify("start 1");
            BigInteger now = Blockchain.GetHeader(Blockchain.GetHeight()).Timestamp;
            Runtime.Notify("start 2"+now);
            
            byte[] check = Storage.Get(Storage.CurrentContext, "Active");
            Runtime.Notify("step  3"+check);
            
            
            if (check.Equals(1) && _orderValues[2] >= now)
            {
                Runtime.Notify("start 3 in if1");
                /* if (!address0.Equals(_orderAddresses[1]) && (!address0.Equals(_orderAddresses[2])) && check1.Equals(0))
                 {
                     return false;
                 }*/
            }
            Runtime.Notify("start 4");
            ...
            Runtime.Notify("start 5",SellerHash,BuyerHash);

            if (basicSigValidations(_orderAddresses) == null)//, _v, _sr, _ss, _br, _bs, SellerHash, BuyerHash) != null)
            {
                Runtime.Notify("at basicSigValidateions");
               // return false;
            }
            Runtime.Notify("start 6" );
            byte[] twc = (ExecutionEngine.ExecutingScriptHash).Concat(SellerHash);
            Runtime.Notify("start 7",twc);
            byte[] twc1 = BuyerHash.Concat(twc).Concat(_orderID);
            Runtime.Notify("start 8", twc1);
            byte[] TWH = Hash256(twc.Concat(twc1));
            Runtime.Notify("start 9", TWH);
            ...
            // if ((orderExists(TWH, _orderID) == true))
            {
              //  if (Main("validExchangeFee", _orderAddresses[3], _orderAddresses[4], _orderAddresses[0], _orderAddresses[1]) != null)

                {
                    ...
                    Runtime.Notify("start 10", fi);
                    byte[] fi1 = (byte[])getFeeIndex(_orderAddresses[4]);
                    Runtime.Notify("start 11", fi1);
                    BigInteger FeeIndex = Neo.SmartContract.Framework.Helper.AsBigInteger(fi);
                    Runtime.Notify("start 12"+FeeIndex);
                    BigInteger FeeIndex1 = Neo.SmartContract.Framework.Helper.AsBigInteger(fi1);
                    Runtime.Notify("start 13"+FeeIndex1);

                    uint FIF = (uint)FeeIndex;
                    Runtime.Notify("start 13 A", FIF);

                    uint FIF1 = (uint)FeeIndex1;
                    Runtime.Notify("start 13 B", FIF1);
                    ...
                    //Seller_Fee_Index
                        ...
                        BigInteger I = new BigInteger(check);
                        BigInteger totalFees = (I * _orderValues[0]);///(1 NEO);

                        Runtime.Notify(totalFees);
                    ...
                    BigInteger I1 = new BigInteger(check);
                    BigInteger totalFees1 = (I * _orderValues[1]);///(1 NEO);

                    Runtime.Notify(totalFees1);




                    _orderValues[0] = totalFees;
                    _orderValues[1] = totalFees1;




                    Runtime.Notify("start 13 C", _orderValues[0]);
                    Runtime.Notify("start 13 D", _orderValues[1]);
                    ...
                    if (_orderValues[0] > 0 && _orderValues[1] > 0 && _orderAddresses[0] != null && _orderAddresses[1] != null && _orderAddresses[2] != null && _orderAddresses[1] != _orderAddresses[2] && _sellerTokens.Length > 0 && _sellerValues.Length > 0 && _sellerTokens.Length == _sellerValues.Length && _buyerTokens.Length > 0 && _buyerValues.Length > 0 && _buyerTokens.Length == _buyerValues.Length)
                    {
                        ...
                        Runtime.Notify(_orderValues[0],_orderValues[1],_orderAddresses[0],_orderAddresses[1],_orderAddresses[1],_orderAddresses[2]);
                        Runtime.Notify(_sellerValues[0],_sellerValues[1],_buyerValues[0],_buyerValues[1]);
